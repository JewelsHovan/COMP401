...
fasta_files = [f for f in os.listdir(fasta_folder) if f.startswith("drosophila_")]
fasta_paths = [os.path.join(fasta_folder, f) for f in os.listdir(fasta_folder) if f.startswith("drosophila_") and f.endswith(".fasta")]
...
def process_annotated_file(filename, fasta_file):
    ...
    with open(filename, "r") as file:
        ...
        species_id = os.path.splitext(os.path.basename(filename))[0].replace("drosophila_", "")
        species_name = species_id_to_name.get(species_id, f"Unkown_{species_id}")
        print(species_name)
        ...
        for annotated_file in annotated_files:
            ...
            if not df.empty and not df[(df['Bias Type'] == 'SINGLE') | (df['Bias Type'] == 'MULTIPLE')].empty:
                ...
                if not filtered_df.empty:
                    filtered_df.to_excel(writer, sheet_name=f"species_{species_id}", index=False)

def get_signature_percentage(filepaths, signature):
    percentages = []
    for filepath in filepaths:
        fasta_file = os.path.join(fasta_folder, os.path.basename(filepath.replace(".out", ".fasta")))
        df = process_annotated_file(filepath, fasta_file)
        total_proteins = df['Sequence Name'].nunique()
        signature_proteins = df[df['Signature'] == signature]['Sequence Name'].nunique()
...
def get_signature_positions(filepaths, signature):
    ...
    for filepath in filepaths:
        ...
        positions = list(zip(signature_df['Start Position'], signature_df['End Position']))
